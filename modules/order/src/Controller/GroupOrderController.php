...
class GroupOrderController extends GroupContentController {
  ...
  public function addPage(GroupInterface $group, $create_mode = FALSE) {
    ...
    $storage_handler = $this->entityTypeManager->getStorage('commerce_order_type');
    foreach ($this->addPageBundles($group, $create_mode) as $plugin_id => $bundle_name) {
      if (!empty($build['#bundles'][$bundle_name])) {
        ...
        $description = $create_mode
          ? $this->t('Create a order of type %order_type in the group.', $t_args)
          : $this->t('Add an existing order of type %order_type to the group.', $t_args);
